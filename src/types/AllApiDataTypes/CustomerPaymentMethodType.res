...
let sortPaymentListArray = (plist, ~customOrder: option<array<string>>=?) => {
  let order = switch customOrder {
  | Some(arr) if arr->Array.length > 0 => arr->Array.toReversed
  | _ => Types.priorityArr
  }
  plist->Array.sort((s1, s2) => {
    let priority1 = order->Array.findIndex(x => x == s1.payment_method_type)
    let priority2 = order->Array.findIndex(x => x == s2.payment_method_type)
